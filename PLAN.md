...
- [x] 修复 WebSocket 断连后重连稳定性（RuntimeError catch + finally 清理）
- [x] 麦克风权限（AVAudioApplication.requestRecordPermission + entitlements 链接）
- [ ] 快捷键启动/停止录音
- [ ] 一键启动脚本（Python 后端 + Swift 前端）
- [x] UI 美化（speaker 标签去重、视觉分组、短句同行、partial/final 光标过渡）
...
> 起因：双路音频（系统音频 + 麦克风）并发处理时 source 变量被覆盖 + ConversationManager 的 pending 状态被两路互相覆盖

### 6.1 ConversationManager 拆分 ✅
- `conversation.py` 拆成 `SharedHistory`（共享对话记录 + context_window + 导出）+ `SentenceSegmenter`（per-speaker 分句器 + 独立 pending 缓冲）
- 两路的 SentenceSegmenter 各自独立，SharedHistory 通过 list.append 共享

### 6.2 AudioSource 独立管道 ✅
- server.py 新增 `AudioSource` 类（原 AudioHandler），每个音频源封装独立的 StreamingSTT + SentenceSegmenter + partial_id
- 主循环只做消息分发（`asyncio.create_task`），音频处理在独立协程里完成
- `_whisper_lock` 序列化 GPU 推理（MLX Metal 不支持并发）

### 6.3 speaker_id 模块 ✅
- `core/speaker_id.py` 管理说话人识别
- 当前简化逻辑：系统音频→"interviewer"，麦克风→"me"
- 为未来 diarization 扩展预留

### 6.4 已知限制
- Voice Processing (AEC) 与 ScreenCaptureKit 的音频设备冲突（AUVPAggregate），无法同时使用
- 麦克风失败不阻塞系统音频（do/catch 隔离）

### 6.5 未来计划
- [ ] 文件夹重组：core/ → audio/ + conversation/ + translation/（import 路径变更）
- [ ] 前端回声抑制：用系统音频 RMS 作为参考，过滤麦克风回声
- [ ] Speaker diarization：在 AudioSource 内部叠加声纹识别模块
...
| 音频管道 | server.py AudioSource | per-speaker 独立管道（StreamingSTT + SentenceSegmenter + partial_id） |
| 对话管理 | conversation.py | SharedHistory（共享记录）+ SentenceSegmenter（per-speaker 分句） |
| 说话人识别 | core/speaker_id.py | 当前：系统音频→interviewer，麦克风→me |
| RAG | LangChain + ChromaDB | 本地 embedding + 检索 + 提示生成 |
| 后端 | FastAPI + WebSocket | uvicorn, 异步, asyncio.create_task 并发 |
| 前端 | Swift + SwiftUI | MenuBarExtra + NSPanel 悬浮窗 + ScreenCaptureKit + AVAudioEngine |
| 通信 | WebSocket | binary(1字节source前缀+音频) + JSON(transcript/translation_delta/translation_final/suggestion) |
